...
# Set the plotting backend for matplotlib. TkAgg is an interface to the Tk GUI toolkit.
matplotlib.use("TkAgg")

# Function to calculate positions after applying trend filters to the entire dictionary of instruments.
# It loops through each instrument in the dictionary and applies the trend filter to its positions.
...
if __name__ == "__main__":
    ...
    # Create foreign exchange series data.
    ...
    # Calculate standard deviations for the instruments.
    ...
    # Calculate the average positions held for each instrument.
    ...
    # Apply trend filters to the positions.
    ...
    # Calculate the percentage returns after costs for each instrument.
    ...
    # Aggregate the returns across instruments.
    perc_return_agg = aggregate_returns(perc_return_dict)

    # Calculate performance statistics for the sp500 instrument and the aggregate returns.
    stats_dict = calculate_stats(perc_return_dict["sp500"])
    stats_dict2 = calculate_stats(perc_return_agg)

    # Convert the statistics to a pandas DataFrame for easier viewing.
